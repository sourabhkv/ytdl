...
def about():
    ...
    streams6 = Label(root2, text = "Youtube-dl GUI VERISION 22.0427.19",bg="#303135",fg="white").place(x = 80,y = 163)
    streams6 = Label(root2, text = "Changelog- faster loading with 1 request",bg="#303135",fg="white").place(x = 60,y = 180)
...
def srch(url):
    global cmbbsc,cmb,cmb2,cmbmus,btn,audlst,vidlst,music,basic,check
    ydl_opts = {}
    global info
    try:
...
def checkcmbo():
    ...
    if len(basicdata)==0 and len(advvid)==0 and len(advaud)==0 and len(advcap)==0 and len(convertaud)==0:
        ...
        if "Mp3 64 kbps" in convertaud:
            a=('yt-dlp_x86  --parse-metadata \"description:(?s)(?P<meta_comment>.+)\" --parse-metadata \"uploader:(?s)(?P<meta_album_artist>.+)\" --parse-metadata \"%(upload_date,release_year).4s:(?P<meta_date>.+)\" --add-metadata --no-mtime -f ba -x --audio-format mp3 --audio-quality 64K -o {} '+url).format(output)
            tv2w = threading.Thread(target=run_command4, args=(a,))
            tv2w.start()
        elif "Mp3 128 kbps" in convertaud:
            a=('yt-dlp_x86  --parse-metadata \"description:(?s)(?P<meta_comment>.+)\" --parse-metadata \"uploader:(?s)(?P<meta_album_artist>.+)\" --parse-metadata \"%(upload_date,release_year).4s:(?P<meta_date>.+)\" --add-metadata --embed-thumbnail --no-mtime -f ba -x --audio-format mp3 --audio-quality 128K -o {} '+url).format(output)
            tv2w = threading.Thread(target=run_command4, args=(a,))
            tv2w.start()
        elif "Mp3 320 kbps" in convertaud:
            a=('yt-dlp_x86  --parse-metadata \"description:(?s)(?P<meta_comment>.+)\" --parse-metadata \"uploader:(?s)(?P<meta_album_artist>.+)\" --parse-metadata \"%(upload_date,release_year).4s:(?P<meta_date>.+)\" --add-metadata --embed-thumbnail --no-mtime -f {}  -x --audio-format mp3 --audio-quality 320K -o {} '+url).format(output)
            tv2w = threading.Thread(target=run_command4, args=(a,))
            tv2w.start()
        elif "M4a High" in convertaud:
            a=('yt-dlp_x86  --parse-metadata \"description:(?s)(?P<meta_comment>.+)\" --parse-metadata \"uploader:(?s)(?P<meta_album_artist>.+)\" --parse-metadata \"%(upload_date,release_year).4s:(?P<meta_date>.+)\" --add-metadata --embed-thumbnail --no-mtime -f ba -x --audio-format m4a --audio-quality 0  -o {} '+url).format(output)
            tv2w = threading.Thread(target=run_command4, args=(a,))
            tv2w.start()
        elif "Wav Lossless" in convertaud:
            a=('yt-dlp_x86  --parse-metadata \"description:(?s)(?P<meta_comment>.+)\" --parse-metadata \"uploader:(?s)(?P<meta_album_artist>.+)\" --parse-metadata \"%(upload_date,release_year).4s:(?P<meta_date>.+)\" --add-metadata --embed-thumbnail --no-mtime -f ba -x --audio-format wav --audio-quality 0 -o {} '+url).format(output)
            tv2w = threading.Thread(target=run_command4, args=(a,))
            tv2w.start()
        elif "Flac 24 bit Lossless" in convertaud:
            a=('yt-dlp_x86  --parse-metadata \"description:(?s)(?P<meta_comment>.+)\" --parse-metadata \"uploader:(?s)(?P<meta_album_artist>.+)\" --parse-metadata \"%(upload_date,release_year).4s:(?P<meta_date>.+)\" --add-metadata --embed-thumbnail --no-mtime -f ba -x --audio-format flac --audio-quality 0 -o {} '+url).format(output)
            tv2w = threading.Thread(target=run_command4, args=(a,))
            tv2w.start()
