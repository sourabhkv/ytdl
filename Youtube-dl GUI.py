...
def captn(url):
    global cmb3,tab1clr,tab2clr,tab4clr,tb2cap
    subs=[]
    if "youtube.com" in url or "youtu.be" in url:
        ...
        for i in range(0,len(ss)):
            if 'auto-generated' not in str(ss[i]):
                a=str(ss[i])[1:-1]
                subs.append(a)
...
def checkcmbo():
    ...
    if len(basicdata)==0 and len(advvid)==0 and len(advaud)==0 and len(advcap)==0 and len(convertaud)==0:
        ...
        #print("adv 123")
        if "youtube.com" in url or "youtu.be" in url:
            u=advcap.replace("\""," ").split()[-1]
        else:
            u=advcap.split()[0]
        print(u)
        a=("yt-dlp_x86 --parse-metadata \"description:(?s)(?P<meta_comment>.+)\" --parse-metadata \"uploader:(?s)(?P<meta_album_artist>.+)\" --write-srt --sub-lang {} --add-metadata --no-mtime --embed-metadata -f {}+{} -o {} "+url).format(u,advvid.split()[0],advaud.split()[0],output)
        #print(a)
        tv2w = threading.Thread(target=run_command4, args=(a,))
        tv2w.start()
