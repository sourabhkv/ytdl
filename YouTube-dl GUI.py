...
name = Label(root, text = "URL").place(x = 45,y = 55)#440
...
def pause():
    ...
    try:
        ...
        sts=popens(b)
        btn['state'] = NORMAL
        name2x = Label(root, text = "Download paused                                                                                                                                                                                                         ",font=('Arial', 10)).place(x = 100,y = 290)
...
def about():
    ...
    streams2 = Label(root2, text = "This is project is based on yt-dlp, ffmpeg, atomic parsley").place(x = 10,y = 30)
    ...
    streams6 = Label(root2, text = "YouTube-dl GUI VERISION 21.261.11").place(x = 80,y = 163)
    streams6 = Label(root2, text = "Changelog- Complete design change, Separate music section").place(x = 10,y = 180)
    streams8 = Label(root2, text = "Creation of file set to current date & time ").place(x = 60,y = 200)
    streams8 = Label(root2, text = "Download progress in statusbar, bug fixes").place(x=60,y=220)
...
def srch(url):
    ...
    if True:
        ...
        for i in range(0,len(a)):
            if "vp9" in a[i] or "video" in a[i] and "[info]" not in a[i] and "[download]" not in a[i] and "[generic]" not in a[i] or "fps" in a[i] or "avc1" in a[i]  or "mp4" in a[i] and "mp4a" not in a[i] and "audio" not in a[i]:
                vidlst.append(a[i])
        ...
        if "no separate audio streams available audio included in video stream" not in audlst:
            ...
            cmb2.place(x=30,y=260)
            streamsaud = Label(root, text = "Audio streams").place(x = 30,y = 230)
            cmb4 = ttk.Combobox(root, width="44", values=music,state="readonly")
            streamsaud = Label(root, text = "Convert to Music").place(x = 340,y = 290)
            cmb4.place(x=337,y=320)
...
def captn(url):
    ...
    if len(subs)==0:
        ...
        cmb3 = ttk.Combobox(root, width="45", values=subs,state="readonly")
...
def run_command2(cmd):
    ...
    #name2x = Label(root, text = "Updated                                                                                                                                                                                                         ",font=('Arial', 10)).place(x = 100,y = 395)
